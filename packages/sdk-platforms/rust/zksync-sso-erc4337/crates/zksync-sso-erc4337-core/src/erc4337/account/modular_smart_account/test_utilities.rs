...
use alloy::{
    ...
    providers::{
        Network, PendingTransactionBuilder, PendingTransactionError, Provider, WalletProvider,
        WatchTxError,
    },
    ...
    P: Provider + WalletProvider + Send + Sync + Clone,
{
    ...
    let pending = ensure_tx_propagated(&provider, &mut fund_tx, pending).await?;
    ...
    N: Network<TransactionRequest = TransactionRequest>,
{
    for rebroadcast_attempt in 0..=TX_REBROADCAST_ATTEMPTS {
        ...
        if missing_from_peers {
            warn!("transaction {tx_hash:?} not found among peers; rebroadcasting");
        } else {
