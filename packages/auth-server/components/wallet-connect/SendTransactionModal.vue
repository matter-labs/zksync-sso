<template>
  <div class="flex flex-col gap-4 flex-1 items-center">
    ...
    <p class="text-sm text-gray-600">
      Requests to send the following transaction:
    </p>
    <pre class="text-xs text-left bg-gray-50 p-3 rounded-lg">{{ transaction }}</pre>
...
const transaction = computed(() => {
  return JSON.stringify(props.request.params.request.params[0], null, 2);
});

const handleSend = async () => {
  await walletConnectStore.sendTransaction(props.request);
  props.closeModal();
};
