use crate::erc4337::account::modular_smart_account::guardian::{
    contract::GuardianExecutor, recovery::RecoveryType,
};
use alloy::{
...
#[derive(Clone)]
pub struct InitializeRecoveryParams<P: Provider + Send + Sync + Clone> {
...
pub async fn initialize_recovery<P>(
    params: InitializeRecoveryParams<P>,
) -> eyre::Result<TransactionReceipt>
where
    P: Provider + Send + Sync + Clone,
{
    let InitializeRecoveryParams {
        guardian_executor,
        account,
        recovery_type,
        data,
    ...
    let receipt = guardian_executor_instance
        .initializeRecovery(account, recovery_type.into(), data)
        .send()
        .await?
        .get_receipt()
        .await?;
...
mod tests {
            ...
            account::{
                ...
                modular_smart_account::{
                    ...
                    guardian::{
                        ...
                        recovery::status::get_recovery_status,
                    },
    ...
    async fn test_initialize_recovery() -> eyre::Result<()> {
        ...
        // Verify recovery was initialized correctly
        {
            ...
            eyre::ensure!(
                recovery_status.is_initialized(),
                "Recovery should be initialized"
            );
        }
