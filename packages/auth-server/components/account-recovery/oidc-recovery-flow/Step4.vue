...
async function go() {
  try {
    ...
    if (startRecoveryReceipt.status !== "success") {
      throw new Error(`Recovery transaction failed with status: ${startRecoveryReceipt.status}. Transaction hash: ${sentTx}`);
    }
    ...
    debugInfo.value = formatDebugInfo(stepName, error, {
