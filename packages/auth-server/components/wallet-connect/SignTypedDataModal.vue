<template>
  <div class="flex flex-col gap-4 flex-1 items-center">
    ...
    <pre class="text-xs text-left bg-gray-50 p-3 rounded-lg ">{{ message }}</pre>
...
const message = computed(() => {
  return JSON.stringify(JSON.parse(props.request.params.request.params[1]).message, null, 2);
});

const handleSign = async () => {
  await walletConnectStore.signTypedData(props.request);
  props.closeModal();
};
