...
// For v0.8 we compute EIP-712 digest from the v0.8 PackedUserOperation fields directly

pub mod pack;

pub use pack::CodeReader;
...
pub async fn get_user_operation_hash_with_provider<P: Provider + Clone>(
    ...
    let code = provider.get_code_at(*entry_point).latest().await?; // not used; fetch sender code instead
    let sender_code =
...
mod tests {
    ...
    fn test_get_user_operation_hash_with_code_reader() {
        ...
        // Calculate and print intermediary values
        use crate::erc4337::user_operation::hash::v08::pack::*;
        ...
        // Expected hash from the TypeScript test
