...
pub fn encode_passkey_signature(
    ...
    type SignatureParams =
        (alloy::sol_types::sol_data::Bytes, alloy::sol_types::sol_data::String, alloy::sol_types::sol_data::FixedArray<alloy::sol_types::sol_data::FixedBytes<32>, 2>, alloy::sol_types::sol_data::Bytes);
...
mod tests {
    ...
    fn test_encode_passkey_signature_matches_ethers() {
        ...
        let test_data_json = include_str!("../../../../../web/test-data/ethers-test-data.json");
        ...
        for test_case in test_cases {
            ...
            match result {
                Ok(rust_encoded) => {
                    println!("Expected (ethers): {}", &test_case.expected[..130.min(test_case.expected.len())]);
                    println!("Got (rust):        {}", &rust_encoded[..130.min(rust_encoded.len())]);

                    let expected_bytes = hex::decode(test_case.expected.trim_start_matches("0x"))
                        .expect("Failed to decode expected hex");
                    let rust_bytes = hex::decode(rust_encoded.trim_start_matches("0x"))
                    ...
                    if rust_encoded != test_case.expected {
                        ...
                        for (i, chunk) in expected_bytes.chunks(32).take(4).enumerate() {
                            println!(
                                "  Word {}: {}",
                                i,
                                hex::encode(chunk)
                        ...
                        for (i, chunk) in rust_bytes.chunks(32).take(4).enumerate() {
                            println!(
                                "  Word {}: {}",
                                i,
                                hex::encode(chunk)
                        ...
                        let min_len = expected_bytes.len().min(rust_bytes.len());
                        ...
                        for i in 0..min_len.min(160) {
                            if expected_bytes[i] != rust_bytes[i] {
                                ...
                                println!("  Byte {}: 0x{:02x} ✅", i, expected_bytes[i]);
                            }
                        }
