...
pub type SignatureProvider =
    Box<dyn FnOnce(FixedBytes<32>) -> eyre::Result<Bytes>>;
...
pub async fn send_transaction_eoa<P: Provider + Send + Sync + Clone>(
    ...
    let signature_provider = Box::new(move |hash: FixedBytes<32>| {
...
mod tests {
    ...
    async fn test_send_transaction_contracts() -> eyre::Result<()> {
        ...
        let response = send_transaction_eoa(
            address,
            eoa_validator_address,
            entry_point_address,
            encoded_calls,
