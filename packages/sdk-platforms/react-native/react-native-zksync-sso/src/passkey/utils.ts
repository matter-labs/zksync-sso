// @ts-ignore
import { initAndroidLogger, initAppleLogger, testLogging, type RpId } from 'react-native-zksync-sso';
...
export const getRpIdString = (rpId: RpId): string => {
    console.log("getRpIdString - rpId:", rpId);
    console.log("getRpIdString - rpId type:", typeof rpId);
    console.log("getRpIdString - rpId structure:", JSON.stringify(rpId, null, 2));
    ...
    console.log("getRpIdString - tag:", tag);
...
export function initializePlatformLogger(appleBundleIdentifier: string): void {
    console.log(`Initializing platform logger for ${Platform.OS}`);

    try {
        if (Platform.OS === 'ios') {
            initAppleLogger(appleBundleIdentifier);
            console.log('iOS logger initialized successfully');
        } else if (Platform.OS === 'android') {
            initAndroidLogger();
            console.log('Android logger initialized successfully');
        } else {
            console.log(`Unsupported platform for logging: ${Platform.OS}`);
            return;
        }
    } catch (error) {
