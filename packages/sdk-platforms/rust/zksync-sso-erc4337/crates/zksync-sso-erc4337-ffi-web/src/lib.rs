...
pub fn encode_passkey_signature(
    ...
    let r_fixed: [u8; 32] = r.try_into().expect(&format!("Failed to convert r to [u8; 32], got length {}", r.len()));
    let s_fixed: [u8; 32] = s.try_into().expect(&format!("Failed to convert s to [u8; 32], got length {}", s.len()));
