...
fn deploy_entrypoint_contract(path: &Path, print_logs: bool) -> Result<()> {
    let mut install_cmd = Command::new("yarn");
    install_cmd.arg("install").current_dir(path);
    run_command(install_cmd, print_logs, "yarn install")?;

    let mut deploy_cmd = Command::new("yarn");
    deploy_cmd.args(["deploy", "--network", "zksyncos"]).current_dir(path);
    run_command(deploy_cmd, print_logs, "yarn deploy --network zksyncos")?;
    Ok(())
}
